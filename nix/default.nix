/**
  ...
  - toHtml: Interactive HTML visualization (WASM-based)
...
let
  ...
  */
  ...
  /**
    Convert graph to WASM visualization data format.
    Nodes are formatted with id, label, group, and optional color.
  */
  toWasmData =
    {
      graph,
  ...
  /**
    Generate interactive HTML visualization using WASM force graph.
    ...
    wasmDistPath
    : Path to the built WASM distribution (from imp.graph package).
  ...
  */
  toHtmlWith =
    ...
    let
      wasmData = toWasmData { inherit graph colors; };
      graphJson = builtins.toJSON wasmData;

      # Read the index.html from the WASM dist
  ...
  /**
    ...
    wasmDistPath
    : Path to the built WASM distribution.
  */
  toHtml =
    { graph, wasmDistPath }:
    toHtmlWith {
  ...
  /**
    ...
    1. With pre-computed graph (for flakeModule - fast, no runtime eval):
       mkVisualizeScript { pkgs, graph, wasmDistPath }

    2. With impSrc and nixpkgsFlake (standalone - runtime eval of arbitrary path):
       mkVisualizeScript { pkgs, impSrc, nixpkgsFlake, wasmDistPath }
    ...
    impSrc
    : Path to imp.lib source (optional, for standalone mode).

    nixpkgsFlake
    : Nixpkgs flake reference string (optional, for standalone mode).

    impGraphSrc
    : Path to imp.graph nix library (defaults to this file's directory).
  ...
  */
  mkVisualizeScript =
    ...
    let
      ...
      htmlOutput = if isStandalone then "" else toHtml { inherit graph wasmDistPath; };
      ...
      standaloneOutput = ''
        ...
        ${pkgs.nix}/bin/nix eval --raw --impure --expr '
          let
            ...
            # Import from imp.lib source structure (src/ directory)
            analyzeLib = import ("${impSrc}" + "/src/analyze.nix") { inherit lib; };
            registryLib = import ("${impSrc}" + "/src/registry.nix") { inherit lib; };
            # Use imp.graph visualization (this file)
            visualizeLib = import "${impGraphSrc}" { inherit lib; };
            ...
            formatted =
              if "'"$FORMAT"'" == "json" then
                ...
                visualizeLib.toHtml { inherit graph; wasmDistPath = "${wasmDistPath}"; };
          in
          formatted
        '
      '';
